...
final class Upcalls {
    ...
    // iOS: the shipped liblwjgl.dylib is built from the 3.3.x sources and does not export
    // ffi_get_closure_size. sizeof(ffi_closure) on aarch64 is 48 (tramp 24 + 3 pointers);
    // 64 covers every 64-bit libffi target and over-allocation is safe (ffi_closure_alloc
    // only uses the leading bytes).
    private static final int CLOSURE_SIZE = 64;
    ...
    private static long getCallbackHandler(Method callback) {
        return Callback.getCallbackHandler(callback);
    }
